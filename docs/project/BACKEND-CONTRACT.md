...
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | /ai/chat | 登录；body {messages:[{role,content}],stream?:bool,max_tokens?:int}。后端代理到 OpenAI 兼容网关：env `AI_BASE_URL`（默认 http://127.0.0.1:8080/v1，即本机 sub2api 网关）、`AI_API_KEY`、`AI_MODEL`（默认 gpt-4o-mini）。stream=true 时原样透传 SSE。未配置 AI_API_KEY → 503 {code:"AI_UNCONFIGURED"}，前端显示真实的「AI 未配置」状态而非假回答 |
