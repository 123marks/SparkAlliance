<template>
  <div class="cp-layout">
    ...
    <!-- 聊天面板 -->
    <main v-if="rightPanel==='chat'" class="cp-main">
      <div class="cp-chat-hdr">
        ...
        <button v-if="activeChat?.type==='private'" class="cp-hdr-btn" @click="showChatFriendCard">👤</button>
        <button v-if="activeChat?.type==='group'" class="cp-hdr-btn" @click="showGroupQR=true">📱</button>
      </div>
      <div class="cp-messages" ref="chatScrollRef">
        <div v-for="msg in chatMessages" :key="msg.id" class="cp-msg" :class="{mine:msg.sender_id===myProfile?.spark_id,ai:msg.sender_type==='ai',sys:msg.type==='system'}">
          ...
          <template v-else>
            <span v-if="msg.sender_id!==myProfile?.spark_id" class="cp-msg-av">{{ msg.sender_avatar }}</span>
      ...
      <div class="cp-input-area">
        ...
        <div class="cp-tools">
          ...
          <button @click="showToast('图片功能开发中')">🖼️</button>
          <button @click="showToast('文件功能开发中')">📎</button>
...
import { useCompanion, formatTimeAgo, type Friend } from '../../composables/useCompanion'
...
function showChatFriendCard(){if(!activeChat.value||activeChat.value.type!=='private')return;const f=friends.value.find(f=>f.spark_id===activeChat.value!.id);if(f){selectedContact.value=f;rightPanel.value='contact'}}
...
void updateProfile;void favorites;void addFavorite;void CosmicBackground;void formatTimeAgo
</script>
